...
public class SealedMessageRetrievalClient {
     ...
     * @deprecated Replaced by signature with CertificateValidator
     */
    ...
    /**
     * Validates the signature of a decrypted payload.
     *
     * @param decryptedSignedPayload the decrypted signed payload from the received sealed message (Mandatory)
     * @param certificateValidator   the CertificateValidator implementation which should be used to verify the certificate
     *                               (Mandatory)
     * @return validSignature <i>true</i> if the signature is valid, otherwise <i>false</i>
     * @throws XspException                   if an XSP error occurs while trying to obtain the signature of the payload.
     * @throws SignatureValidationException   if a Signature Validation exception error occurs while trying
     *                                        to obtain the signature of the payload.
     * @throws CertificateValidationException if a Certificate Verifier exception error occurs
     */
    public static boolean verifyPayloadSignature(Document decryptedSignedPayload, CertificateValidator certificateValidator)
            throws XspException, SignatureValidationException, CertificateValidationException {
        ArgumentUtils.checkNotNull(decryptedSignedPayload, "decryptedSignedPayload");
        ArgumentUtils.checkNotNull(certificateValidator, "certificateValidator");

        return SignedContainerProfileUtil.verifySignature(decryptedSignedPayload, certificateValidator);
    }
