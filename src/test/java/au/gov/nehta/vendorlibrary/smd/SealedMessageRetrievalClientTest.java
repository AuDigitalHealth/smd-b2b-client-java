...
public class SealedMessageRetrievalClientTest {
    ...
    public void testListAndRetrieveOnDrp() throws Exception {
        ...
        Assert.assertNotEquals(testClient.getLastSoapRequest(), LoggingHandler.EMPTY);
        Assert.assertTrue(testClient.getLastSoapRequest().length() > 0);

        Assert.assertNotNull(testClient.getLastSoapResponse());
        Assert.assertNotEquals(testClient.getLastSoapResponse(), LoggingHandler.EMPTY);
        ...
        List<String> invocationIdentifiers = new ArrayList<>();
        ...
        // Changed to use CertificateValidator implementation instead of CertificateVerifier, which is deprecated.
        boolean validSignature = testClient.verifyPayloadSignature(decryptedSignedPayload, SignedContainerProfileUtil.NULL_CERTIFICATE_VALIDATOR);
        ...
        Assert.assertEquals(retrievedPayloadString, payload);
    }
    ...
    public void doDeliverOnDrp(SealedMessageType sealedMessage) throws Exception {
        ...
        Assert.assertSame(deliverStatusType, DeliverStatusType.OK);
    ...
    private MessageMetadataType getDefaultMessageMetadata() {
        ...
        List<ElsCertRefType> certRefList = new ArrayList<>();
        ...
        List<RouteRecordEntryType> routeRecords = new ArrayList<>();
        ...
        List<OtherTransportMetadataEntryType> otherTransportMetadataEntryTypes = new ArrayList<>();
    ...
    private SealedMessageType getSealedMessageForXmlStringPayload(MessageMetadataType metadata, String unencryptedXmlPayload) throws GeneralSecurityException, UnsupportedEncodingException, JAXBException {
        ...
        X500PrivateCredential privateCredential = new X500PrivateCredential(senderCert, senderPrivateKey);
        List<X500PrivateCredential> certificatePairs = new ArrayList<>();
